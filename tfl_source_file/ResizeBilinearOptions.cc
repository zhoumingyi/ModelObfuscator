...
//   const int32* size_data = GetTensorData<int32>(size);
  const int32* size_data = size_raw;
...
//   TF_LITE_ENSURE_OK(context, GetInputSafe(context, node, kSizeTensor, &size));
  ...
  TF_LITE_ENSURE_EQ(context, size_dims_size, 1);
...
#define TF_LITE_RESIZE_BILINEAR(type, opname, datatype)              \
  ...
  type::opname(op_params, GetTensorShape(input),                     \
               GetTensorData<datatype>(input), RuntimeShape(size_dims_size, size_dims_data), \
               size_data, GetTensorShape(output),   \
...
TfLiteRegistration* Register_randopname_REF() {
  static TfLiteRegistration r = {
      nullptr, nullptr, randopname::Prepare,
...
TfLiteRegistration* Register_randopname() {
  static TfLiteRegistration r = {
      nullptr, nullptr, randopname::Prepare,
      randopname::Eval<randopname::kOptimized>};
  return &r;
}
