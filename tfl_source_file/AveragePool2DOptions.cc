...
//   TF_LITE_ENSURE(context, params->stride_width > 0);
  TF_LITE_ENSURE(context, stride_height > 0);
  TF_LITE_ENSURE(context, stride_width > 0);
...
//       &out_width);
  data->padding = ComputePaddingHeightWidth(
      ...
      filter_height, filter_width, padding, &out_height,
      &out_width);
...
TfLiteStatus AverageEvalQuantizedUint8(TfLiteContext* context, TfLiteNode* node,
  ...
  (void)CalculateActivationRangeQuantized(context, activation, output,
...
TfLiteStatus AverageEvalQuantizedInt16(TfLiteContext* context, TfLiteNode* node,
  ...
  int32_t activation_min;
  int32_t activation_max;
  CalculateActivationRangeQuantized(context, activation, output,
                                    &activation_min, &activation_max);
#define TF_LITE_AVERAGE_POOL(type)                                            \
  tflite::PoolParams op_params;                                               \
  op_params.stride_height = stride_height;                            \
...
TfLiteRegistration* Register_randopname_REF() {
  static TfLiteRegistration r = {randopname::Init, randopname::Free,
                                 randopname::GenericPrepare<randopname::kAverage>,
                                 randopname::AverageEval<randopname::kReference>};
  return &r;
}
...
TfLiteRegistration* Register_randopname_GENERIC_OPT() {
  static TfLiteRegistration r = {
      randopname::Init, randopname::Free, randopname::GenericPrepare<randopname::kAverage>,
      randopname::AverageEval<randopname::kGenericOptimized>};
  return &r;
}
